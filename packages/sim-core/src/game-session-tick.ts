...
import type { GameSessionDiagnostics } from "./game-session-diagnostics";
...
export class GameSessionTickCoordinator {
  ...
  private readonly needDeltas = new Int32Array([-1, -1, 0, 0, 0]);
  ...
  constructor(
    ...
    diagnostics: GameSessionDiagnostics,
  ...
  runOneTick(tick: Tick): void {
    ...
    this.enterPhase(7);
    this.diagnostics.auditTransientLeaks(tick);
    this.enterPhase(8);
  }
  ...
  createMetrics(
    ...
    return {
      ...
      activeJobCount: jobMetrics.activeCount,
  ...
  private refreshBoundedDirtyQueues(): void {
    ...
    if (this.owners.needUrgency.createMetrics().dirtyBacklog > 0) {
      this.owners.needUrgency.refreshDirty(this.owners.needs, 4);
    }
  }
  ...
  private advanceTownOwners(tick: Tick): void {
    this.owners.needs.processScheduledUpdates(tick, this.needDeltas, 4, this.owners.needUrgency);
    this.owners.needUrgency.refreshDirty(this.owners.needs, 4);
