...
export interface Pathing100BenchmarkReport {
  readonly name: "pathing-100";
...
export interface Pathing100BenchmarkInvariants extends Record<string, boolean | number | string> {
  readonly width: number;
  readonly height: number;
  readonly requestCount: number;
  readonly staleProbeCount: number;
...
export interface SampledPathing100Benchmark {
  readonly name: "pathing-100";
  readonly report: Pathing100BenchmarkReport;
  readonly invariants: Pathing100BenchmarkInvariants;
  readonly sampleElapsedMs: readonly number[];
  readonly stats: BenchmarkSampleStats;
}

export function runPathing100Benchmark(): Pathing100BenchmarkReport {
  const width = 32;
  const height = 32;
  ...
  while (batcher.queuedCount > 0) {
    ...
    if (!processed.ok || !processed.processed) {
      throw new Error(processed.ok ? "missing pathing benchmark result" : processed.reason);
    }
  ...
  if (!staleProcessed.ok || !staleProcessed.processed) {
    throw new Error(staleProcessed.ok ? "missing stale path result" : staleProcessed.reason);
  }
  ...
  if (staleCommitted.ok) {
    throw new Error("pathing stale probe unexpectedly committed");
  }
...
export function pathing100InvariantsFromReport(
...
function drainRegionRoom(rebuild: ReturnType<typeof createRegionRoomRebuilder>): void {
  ...
  throw new Error("pathing benchmark region-room drain guard exhausted");
}
