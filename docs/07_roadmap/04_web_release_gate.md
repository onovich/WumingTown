...
WM-0096 records the M6 Web tier verdict as `demo-only`.

This is not a Web cancellation decision. Web remains a gated target, but the
current M6 evidence does not prove `same-spec`, `lower fast-forward` or
`lower cap`, because those claims require a measured browser authority runtime
instead of the current product-gate shell plus Worker projection path.

Decision inputs:

- Chrome Stable and Edge Stable load and interact with the M5 product-gate
  shell.
- Runtime deliverable estimated gzip bytes are `273561`, under the 150 MB
  budget.
- Web save/export/import, diagnostics, input and SAB-unavailable fallback are
