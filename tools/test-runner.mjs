...
function selectVitestTargets(selectedMode, selectedFilter) {
  if (selectedMode === "unit") {
    ...
    if (selectedFilter === "m4-save-replay") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-core/src/m4-save-replay.test.ts"],
      };
    }

    if (selectedFilter === "m5-save-replay") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-core/src/m5-save-replay.test.ts"],
      };
    }

    if (selectedFilter === "m8-save-replay") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-core/src/m8-save-replay.test.ts"],
      };
    }
    ...
    if (selectedFilter === "m4-worker-parity") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-worker/src/m4-worker-parity.test.ts"],
      };
    }

    if (selectedFilter === "m5-worker-parity") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-worker/src/m5-worker-parity.test.ts"],
      };
    }

    if (selectedFilter === "m5-invariants") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: [
    ...
    if (selectedFilter === "m4-invariants") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: [
          "packages/sim-core/src/m4-invariants.test.ts",
          "packages/sim-worker/src/m4-worker-parity.test.ts",
        ],
      };
    }
    ...
    if (selectedFilter === "diagnostics") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/platform/src/diagnostics.test.ts"],
      };
    }
    ...
    if (selectedFilter === "m5-alpha-scenario") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-core/src/m5-alpha-content-scenario.test.ts"],
      };
    }
    ...
    if (selectedFilter === "m5-third-knock") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-core/src/m5-third-knock.test.ts"],
      };
    }

    if (selectedFilter === "m5-old-bridge") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-core/src/m5-old-bridge.test.ts"],
      };
    }

    if (selectedFilter === "m5-faction-governance") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-core/src/m5-faction-governance.test.ts"],
      };
    }

    if (selectedFilter === "m5-season-events") {
      return {
        ok: true,
        extraArgs: ["--exclude=**/*.e2e.test.ts"],
        targets: ["packages/sim-core/src/m5-season-events.test.ts"],
      };
    }
